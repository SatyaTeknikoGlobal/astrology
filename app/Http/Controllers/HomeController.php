 ...
 public function settings(Request $request)
    {
        ...
        if(is_numeric($id) && $id > 0)
        {
            ...
            if(empty($settings))
            {
                return redirect($this->ADMIN_ROUTE_NAME.'/settings');
            }
        }     
        ...
        if($request->method() == "POST" || $request->method() == "post")
        {
            if(empty($back_url))
            {
                 $back_url = $this->ADMIN_ROUTE_NAME.'/settings';
            }
            ...
            {                 
                ...
                $details['refer_earn_amount'] = 'required';  
                $details['about_us'] = 'required';  
                $details['privacypolicy'] = 'required';  
                $details['contact_email'] = 'required'; 
                $details['contact_phone'] = 'required';   
                $details['app_name'] = 'required';  
                $details['terms'] = 'required';  
              
            }
          $this->validate($request , $details);
           $createdDetails = $this->saveSettings($request , $id);
           if($createdDetails)
           {
                $alert_msg = "Setting Created Successfully";
                if(is_numeric($id) & $id > 0)
                {
                    $alert_msg = "Setting Updated Successfully";
                } 
        ...
        $page_Heading = "Add Setting";     
        if(is_numeric($id) && $id > 0){
            $page_Heading = 'Update Setting';
        }
        $details['page_Heading'] = $page_Heading;       
        $details['settings'] = $settings; 
       return view('admin.settings.form',$details);
    }
